package org.delivery.db.usrorder;

import org.delivery.db.usrorder.enums.UserOrderStatus;
...
public interface UserOrderRepository extends JpaRepository<UserOrderEntity, Long> {
    ...
    // 특정 유저의 모든 주문
    // select * from user_order where user_id = ? and status = ?  order by id dsec
    List<UserOrderEntity> findAllByUserIdAndStatusOrderByIdDesc(Long userId, UserOrderStatus status);

    // select * from user_order where user_id = ? and status in (?,? ..)  order by id dsec
